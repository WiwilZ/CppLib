...
namespace Detail {
    ...
    using CondResult_T = decltype(false ? ::DeclVal<T1>() : ::DeclVal<T2>());
    ...
    template <typename T1, typename T2>
    struct DoubleCommonType<T1, T2, Void_T<CondResult_T<Decay_T<T1>, Decay_T<T2>>>> : DecayedCondResult<Decay_T<T1>, Decay_T<T2>> {};
}


template <typename T1, typename T2>
struct CommonType<T1, T2> : Conditional<
        IsSame_V<T1, Decay_T<T1>> && IsSame_V<T2, Decay_T<T2>>,
