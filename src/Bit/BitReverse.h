...
namespace Bit {
    ...
    [[nodiscard]] constexpr T BitReverse(T x) noexcept {
        ...
        if constexpr (sizeof(T) <= 2) {
            return Detail::Common::BitReverse(ux);
        } else {
#if defined(__clang__)
            if constexpr (sizeof(T) == 4) {
                return __builtin_bitreverse32(ux);
            } else if constexpr (sizeof(T) == 8) {
                return __builtin_bitreverse64(ux);
            } else {
                static_assert(sizeof(T) == 16, "Unexpected integer size");
                return (static_cast<__uint128_t>(__builtin_bitreverse64(ux)) << 64) | __builtin_bitreverse64(ux >> 64);
            }
#elif defined(__GNUC__)
            if constexpr (sizeof(T) == 4) {
                return Detail::BSwap::BitReverse(__builtin_bswap32(ux));
            } else if constexpr (sizeof(T) == 8) {
                return Detail::BSwap::BitReverse(__builtin_bswap64(ux));
            } else {
                static_assert(sizeof(T) == 16, "Unexpected integer size");
#   if __has_builtin(__builtin_bswap128)
                ux = __builtin_bswap128(ux);
#   else // !__has_builtin(__builtin_bswap128)
                ux = (static_cast<__uint128_t>(__builtin_bswap64(ux)) << 64) | __builtin_bswap64(ux >> 64);
#   endif
...
#   endif
                return Detail::BSwap::BitReverse(ux);
            }
#else // !defined(__clang__) && !defined(__GNUC__)
#   if defined(_MSC_VER)
            if (!__builtin_is_constant_evaluated()) {
                if constexpr (sizeof(T) == 4) {
                    return Detail::BSwap::BitReverse(static_cast<uint32_t>(_byteswap_ulong(ux)));
                } else {
                    static_assert(sizeof(T) == 8, "Unexpected integer size");
                    return Detail::BSwap::BitReverse(_byteswap_uint64(ux));
                }
            }
#   endif
            return Detail::Common::BitReverse(ux);
