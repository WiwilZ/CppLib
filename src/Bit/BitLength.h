...
#endif // defined(BL_USE_LZCNT_BSR_INTRINSICS)
...
namespace Bit {
    ...
    [[nodiscard]] constexpr int BitLength(T x) noexcept {
        ...
        if constexpr (sizeof(T) <= 4) {
            return 32 - __builtin_clz(ux);
        } else if constexpr (sizeof(T) == 8) {
            return 64 - __builtin_clzll(ux);
        } else {
...
#   if defined(BL_USE_LZCNT_BSR_INTRINSICS)
        if (!__builtin_is_constant_evaluated()) {
            return Detail::X86_X64::BitLength(ux);
        }
#   elif defined(BL_USE_CLZ_INTRINSICS)
        if (!__builtin_is_constant_evaluated()) {
            return Detail::Arm_Arm64::BitLength(ux);
        }
#   endif
        return Detail::Common::BitLength(ux);
#endif
    }
